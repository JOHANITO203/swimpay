...
| Landing favicon | `apps/landing/public/favicon.svg` | Browser favicon for `www.swimpay.pro` | Derived from the official monochrome launcher symbol. |
| Landing app icons | `apps/landing/public/swimpay-icon-192.png`, `apps/landing/public/swimpay-icon-512.png`, `apps/landing/public/apple-touch-icon.png` | Web manifest, install preview and Apple touch icon | Black/graphite tile with white SwimPay symbol; must stay reconciled with Android launcher. |
| Landing brand SVGs | `apps/landing/public/brand/swimpay-icon.svg`, `apps/landing/public/brand/swimpay-symbol.svg` | Landing header/footer and web brand surfaces | Web-ready official brand assets derived from the launcher/notification symbol geometry. |
| Landing social preview | `apps/landing/public/images/swimpay-og.png` | Open Graph / Twitter preview for `www.swimpay.pro` | Reconciled with the monochrome launcher identity. |
...
| Landing `BrandMark` | `apps/landing/src/components/BrandMark.tsx` | Landing navbar/footer brand | Renders the official web icon asset and prevents scattered text-only or generic `S` logo variants. |
| Checkout inline SwimPay launcher mark | `apps/web/src/screens/CheckoutScreen.ts` | Buyer checkout header brand only | Runtime-used inline SVG aligned to the official Android launcher symbol. It is not a new asset file and must not become a new resource logo file. The buyer SDK button remains a separate frozen surface. |

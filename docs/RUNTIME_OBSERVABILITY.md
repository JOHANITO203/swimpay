...
Sprint 4I adds receiver diagnostics for the synthetic listener path. The debug status surface may show listener state, synthetic debug source status, outbox pending/retry counts, backend reachability and safe last status/error. It must not show raw phone numbers, raw notification text, raw title/body fields or secrets.

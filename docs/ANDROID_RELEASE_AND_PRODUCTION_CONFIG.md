...
- Receiving-method add form opens immediately in the visible area after `Ajouter une carte` or `Ajouter telephone SBP`.
...
- Android Merchant `Creer cle API` requires the API backend fix from `.swimpay-agent/ANDROID_INTEGRATION_API_KEY_MOBILE_FIX_REPORT.md`.
...
- Android export/copy text must include show-once developer secrets only after phone security, and must not include the mobile bearer token.
...
- `Parametres > Securite` links Google to an existing Android merchant profile;

...
- RECEIVER-SIGN-1 migrated real Receiver signal signing to Android Keystore asymmetric identity. The Android app registers a PEM public key, keeps the EC P-256 private key on-device, signs redacted payloads with `SHA256withECDSA`, and the backend verifies signatures using `ecdsa_p256_sha256_der_v1`. Shared HMAC receiver keys are debug-only compatibility and are rejected for real signal upload.
...
- Receiver signal signature handling now uses a typed `ecdsa_p256_sha256_der_v1` verification result, blocks unknown and disabled receiver devices before persistence, rejects shared receiver verification keys, validates `payload_hash`, and preserves local counter replay protection.
...
- Sprint 4C added Android emulator smoke diagnostics and reports. `adb` is available from the SDK and the debug APK exists, but Android Emulator, AVDs and running devices are unavailable, so APK install and live app smoke validation are blocked and not claimed as passed.

...
Sprint 6A pauses production/admin hardening and creates the five-bank MVP validation foundation. Only Sberbank has real package evidence (`ru.sberbankmobile`); the other V1 banks still require explicit operator package-name input. Real bank notifications remain out of scope until a later approved shadow/review-only sprint. Auto-confirm remains disabled for real banks.

Sprint 6E prepares the real-notification shadow readiness gate. The safe defaults are `SWIMPAY_REAL_NOTIFICATION_SHADOW_ENABLED=false`, `SWIMPAY_REAL_BANK_AUTO_CONFIRM=false` and `SWIMPAY_RAW_NOTIFICATION_STORAGE=false`. Real notification shadow requires explicit operator and merchant consent, review-only bank selection, Notification Listener Access, backend health and outbox health. Shadow auto-confirm prediction is metadata only and must not mutate orders, emit `payment.confirmed` or release fulfillment.

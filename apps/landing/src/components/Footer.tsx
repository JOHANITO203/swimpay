    ...
    <footer className="relative border-t border-white/6 bg-brand-900 py-12">
      <div className="mx-auto flex max-w-6xl flex-col items-center justify-between gap-8 px-4 sm:px-6 md:flex-row">
        <div className="flex flex-col items-center gap-4 md:items-start">
          ...
          <p className="max-w-xs text-center text-sm leading-6 text-slate-500 md:text-left">
        ...
        <div className="flex flex-col items-center gap-5 md:items-end">
          <div className="flex flex-wrap justify-center gap-6 text-sm text-slate-400">
            <a href="#" className="transition-colors hover:text-accent">Politique de confidentialité</a>
            <a href="#" className="transition-colors hover:text-accent">Conditions d'utilisation</a>
          </div>
          <p className="text-xs text-slate-600">© {new Date().getFullYear()} SwimPay. Non affilié à un système bancaire officiel.</p>
        </div>
      </div>
    </footer>
  );

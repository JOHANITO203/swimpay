...
class PremiumAccountEntryStaticTest {
    ...
    fun accountEntryVisualGrammarKeepsTouchTargetsAndNoEmojiIcons() {
        ...
        assertTrue("account entry logo must reuse the official launcher foreground for brand consistency", components.contains("painterResource(R.mipmap.ic_launcher_foreground)"))
        assertTrue("account entry must use the shared launcher badge", source.contains("SwimPayLauncherBadge(size = 82.dp)"))
        assertFalse("account entry must not keep a local launcher badge copy", source.contains("private fun SwimPayLauncherBadge"))
        assertFalse("onboarding must not keep a local launcher badge copy", onboardingSource.contains("private fun SwimPayLauncherBadge"))
        assertFalse("Compose badges must not load deprecated launcher vectors", launcherBadgeSources.contains("painterResource(R.drawable.ic_launcher_foreground)"))

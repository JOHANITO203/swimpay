...
- Replaced fake payment detail timestamp `Signal re�u � Il y a 2 min` with backend timestamp labels or `Signal non horodat�`.
...
- Changed runtime bottom nav to 5 mockup-style tabs: Accueil, En attente, R�cepteurs, Int�grations, Param�tres.
...
- Wired main `Récepteurs` tab to receiving-method create/edit/disable/default/delete runtime callbacks.
- Wired main `Paramètres`/security surface to app-lock, timeout and Google account-link callbacks.
...
- Confirmed current restoration blocker: `Orders/Ventes` remains hidden and the Settings `Ventes` row currently misroutes to `Récepteurs`.
...
- Replaced developer-console labels with merchant-facing `Site connecté`, `Intégration`, `Tester l'intégration`, `Voir guide`.
- Moved API key, webhook secret and URL detail rows behind a collapsed `Détails techniques` section.
...
- Fixed Settings `Ventes` row to open the existing orders feature instead of `Récepteurs`.
...
- 2026-05-17 03:05 +03:00: Restored the intended secure developer-copy behavior for Android integration credentials. After phone security, Android can copy show-once API/webhook secrets for the merchant's developer, while visible rows stay masked and the mobile bearer token is never copied.

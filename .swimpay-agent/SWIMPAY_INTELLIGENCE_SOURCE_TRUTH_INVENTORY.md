...
| Android non-debug upload | `SignalUploadWorker.kt` | partially aligned / dangerous before real testing | Non-debug transport is fail-safe/no-op; real accepted redacted signals do not yet reach staging backend. |
...
1. Wire a non-debug Android staging upload transport that sends only redacted signed envelopes to `/v1/receiver/signals`.
2. Replace synthetic/debug hash label vocabulary in real Android runtime hashes.
3. Remove or rename active admin/operator `auto_confirm_allowed_by_template` and `autoConfirmStatus` vocabulary so operator surfaces cannot imply automatic V1 confirmation.

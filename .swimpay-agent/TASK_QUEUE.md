# SwimPay Task Queue

The runner may prepare only one pending task at a time.

If a task file exists in `tasks/`, it can be prepared. If the root task file is missing, it is marked `missing` and is not automatic.

## Queue

- [x] `381_android_merchant_ux_language_contract` - status: completed - source: `tasks/381_android_merchant_ux_language_contract.md`
- [x] `382_android_onboarding_frontend_screens` - status: completed - source: `tasks/382_android_onboarding_frontend_screens.md`
- [x] `383_android_notification_access_gate` - status: completed - source: `tasks/383_android_notification_access_gate.md`
- [x] `384_android_bank_selection_and_receiving_method_setup` - status: completed - source: `tasks/384_android_bank_selection_and_receiving_method_setup.md`
- [x] `385_android_configuration_test_screen` - status: completed - source: `tasks/385_android_configuration_test_screen.md`
- [x] `386_android_merchant_dashboard_screen` - status: completed - source: `tasks/386_android_merchant_dashboard_screen.md`
- [x] `387_android_receiving_methods_screen` - status: completed - source: `tasks/387_android_receiving_methods_screen.md`
- [x] `388_android_review_queue_and_payment_detail` - status: completed - source: `tasks/388_android_review_queue_and_payment_detail.md`
- [x] `389_android_connected_site_webhook_screen` - status: completed - source: `tasks/389_android_connected_site_webhook_screen.md`
- [x] `390_android_receiver_health_and_settings_shell` - status: completed - source: `tasks/390_android_receiver_health_and_settings_shell.md`
- [x] `391_android_api_contracts_and_state_models` - status: completed - source: `tasks/391_android_api_contracts_and_state_models.md`
- [x] `392_android_ui_tests_copy_guardrails` - status: completed - source: `tasks/392_android_ui_tests_copy_guardrails.md`
- [x] `393_sprint_7d_closeout_review` - status: completed - source: `tasks/393_sprint_7d_closeout_review.md`

## Recently Completed

- [x] `374_copy_details_session_hardening` - status: completed - source: `tasks/374_copy_details_session_hardening.md`
- [x] `375_copy_details_rate_limit_and_audit` - status: completed - source: `tasks/375_copy_details_rate_limit_and_audit.md`
- [x] `376_copy_details_short_lived_reveal_policy` - status: completed - source: `tasks/376_copy_details_short_lived_reveal_policy.md`
- [x] `377_merchant_receiving_route_admin_ui` - status: completed - source: `tasks/377_merchant_receiving_route_admin_ui.md`
- [x] `378_hosted_checkout_browser_qa` - status: completed - source: `tasks/378_hosted_checkout_browser_qa.md`
- [x] `379_checkout_destination_copy_e2e_tests` - status: completed - source: `tasks/379_checkout_destination_copy_e2e_tests.md`
- [x] `380_sprint_7c_closeout_review` - status: completed - source: `tasks/380_sprint_7c_closeout_review.md`
- [x] `350_hybrid_receiving_route_model` - status: completed - source: `tasks/350_hybrid_receiving_route_model.md`
- [x] `351_receiving_route_storage_and_api` - status: completed - source: `tasks/351_receiving_route_storage_and_api.md`
- [x] `352_buyer_sender_phone_matching_hint` - status: completed - source: `tasks/352_buyer_sender_phone_matching_hint.md`
- [x] `353_human_readable_payment_reference_generator` - status: completed - source: `tasks/353_human_readable_payment_reference_generator.md`
- [x] `354_checkout_bank_first_route_reveal_ui` - status: completed - source: `tasks/354_checkout_bank_first_route_reveal_ui.md`
- [x] `355_hybrid_route_matching_risk_policy` - status: completed - source: `tasks/355_hybrid_route_matching_risk_policy.md`
- [x] `356_webhook_route_context_no_pii` - status: completed - source: `tasks/356_webhook_route_context_no_pii.md`
- [x] `357_hybrid_receiving_routes_e2e_tests` - status: completed - source: `tasks/357_hybrid_receiving_routes_e2e_tests.md`
- [x] `358_sprint_7b_closeout_review` - status: completed - source: `tasks/358_sprint_7b_closeout_review.md`
